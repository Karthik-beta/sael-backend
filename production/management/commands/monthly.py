...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            monthly_target_instance.w1_shift = float(df.iloc[3, 1])
            print("w1: ", monthly_target_instance.w1_shift)

            monthly_target_instance.w2_shift = float(df.iloc[4, 1])

            monthly_target_instance.w3_shift = float(df.iloc[5, 1])

            monthly_target_instance.w4_shift = float(df.iloc[6, 1])

            monthly_target_instance.shift_total = float(df.iloc[7, 1])
            ...
            monthly_target_instance.today_rate = float(df.iloc[3, 4])
