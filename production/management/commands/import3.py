...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            shift_name = str(df.iloc[0, 2])
            print("shift_name: ", shift_name)

            if shift_name == 'Shift : A':
                shift = 'FS'
            else:
                shift = 'SS'
            ...
            if shift == 'FS':
                print("shift: ", shift)
                # Define spell stages and their corresponding row indices
                spell_stages = ['First Spell', 'Second Spell', 'Third Spell', 'Fourth Spell']
                row_indices = [3, 4, 5, 6]
                ...
                for stage, row_index in zip(spell_stages, row_indices):
                    ...
                    if spell_data:
                        ...
                        self.stdout.write(self.style.ERROR(f'Error: No data found for {stage} on the specified date, stage, and shift'))
                ...
                for stage, row_index in zip(spell_stages, row_indices):
                    spell_data = spellAssemblyLineData.objects.filter(date=excel_date, stage=stage, shift='SS').last()
                    spell_value = str(df.iloc[row_index, 2])  # Assuming the date is in the first row, second column
